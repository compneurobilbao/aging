function [num_mods_struct,num_mods_func,splitted_SC,splitted_FC] = total()
...
for nMod=21:1000
    ...
    FC_Mod_pil_total_degree = squeeze(sum(FC_Mod_pil,2));

    for i=1:nMod
        [r,p]=corrcoef(SC_Mod_pil_total_degree(i,:)',gainSubj);
        EDGEcn{nMod}(i)=r;
        EDGEpn{nMod}(i)=p;
        [r,p]=corrcoef(FC_Mod_pil_total_degree(i,:)',gainSubj); %ageSubj
        EDGEcnRS{nMod}(i)=r;
        EDGEpnRS{nMod}(i)=p;
    end
...
save -mat7-binary data_total_21_1000.mat newPrevMod EDGEcn EDGEpn EDGEcnRS EDGEpnRS edge_FC edge_SC ...
