...
repository into your computer and follow the instructions at the [bottom of this page](#using-aging-as-a-template).
...
`aging/aging.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `aging.py` in directory called
`aging`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import aging as sb` in an
interactive Python session, the classes and functions defined inside of the
`aging.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the aging module, the main classes defined at the bottom of
...
Either way, you can create a `aging/data` folder in which you can
...
(typically, something like our `aging/tests/test_aging.py`). Within each
...
have one such test in `aging/tests/test_aging.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/aging`),
and type:
...
will see a message such as:
  ...
  aging/tests/test_aging.py:49: AssertionError
...
To document `aging` we use the [sphinx documentation
...
A `aging/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/aging) for the software. This
...
`aging` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `aging`, we use the
...
Next, you will want to move `aging/aging.py` to be called
`smallish/smallish.py`

	git mv aging smallish
	git mv smallish/aging.py smallish/smallish.py
	git mv smallish/tests/test_aging.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `aging` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `aging` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `aging` in them. Type
...
Edit all the mentions of `aging` in the `aging/__init__.py` file, and in
the `aging/version.py` file as well.
